<template>
	<div class="allnetgame">
		...
		<div class="allnetgame-list">
			<ul class="game-list" v-show="danyi">
			...
			<ul class="game-list" v-show="!danyi">
				<li v-for='(item,index) in prj' :data-id='item.id' @click="allgameQu(item.game_name,item.id,item.game_icon)">
					<img :src="item.game_icon" alt="" />
					<p>{{item.game_name}}</p>
				</li>
	...
	export default {
		...
		methods: {
			...
			changLetter(t) {
				this.danyi = false;
				let pitem =[];
				this.gamlist.filter(function(item){
					if(item.game_sp.substr(0, 1).toUpperCase() == t){
					pitem.push(item)
					}					
				})
				if(pitem == ''){
			...
			allgameQu(v, t, img) {
				...
				this.$router.push('/yibi/yibiarea')
